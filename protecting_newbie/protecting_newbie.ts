...
//2차배포가능합니다. 단 코드 마음데로 수정 금지입니다.(사용여부는 수정 가능하십니다.)
...
command.register("뉴비삭제", "뉴비 삭제를 합니다.").overload((p, o, output) => {
    ...
    if (o.getEntity()?.hasTag('new')) {
        ...
        runCommand(`tellraw @a[name="${username}"] {"rawtext":[{"text":"§d§b[ Leesw123 ]§r§6 : 뉴비보호가 해제됐습니다."}]}`)
    } else {
        runCommand(`tellraw @a[name="${username}"] {"rawtext":[{"text":"§d§b[ Leesw123 ]§r§6 : ${username}님은 뉴비가 아니십니다."}]}`)
    }
}, {});


command.register("뉴비플러그인", "뉴비 플러그인 계발자(Leesw123).").overload((p, or, output) => {
    const ni = or.getEntity()?.getNetworkIdentifier()!;
    const actor = ni.getActor()!;
    const username = actor.getName();
    runCommand(`tellraw @a[name="${username}"] {"rawtext":[{"text":"§l§f[ §bLeesw123 §f]§f§6 본 서버는 Leesw123의 뉴비보호 플긴이 작동중 입니다.(도와주신분 이슈님 감사합니다.)"}]}`);
    return CANCEL
}, {});
